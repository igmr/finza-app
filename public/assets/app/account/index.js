...
const btnInfo = async (id) => {
    window.location.href = `${baseUrlAccount}/${id}`;
    return;
};
...
const buildItem = (data) => {
    ...
    return `
            <div class="nk-tb-item" onclick="btnInfo(${data.account_id})">
                <div class="nk-tb-col">
                    <span>${data.account_id}</span>
                </div>
                <div class="nk-tb-col">
                    <span>${data.account ?? ""}</span><br>
                    <small class="d-md-none fw-lighter">${data.bank}</small>
                </div>
                <div class="nk-tb-col tb-col-md">
                    <span>${data.bank}</span>
                </div>
...
const reloadList = async (url) => {
    ...
    let content = buildHeader();
    const response = await getAllAccounts(url);
    detail.innerHTML = "";
    response.data.forEach((item) => {
        content += buildItem(item);
    });
