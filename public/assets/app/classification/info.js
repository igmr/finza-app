const table = new NioApp.DataTable(".info", {
    ...
    ajax: `${baseUrlClassification}/detail/${getId()}`,
    columns: [
        {
            title: "Account/Bank",
            data: null,
...
btnRestore.addEventListener("click", async () => {
    const response = await restoreClassification(classificationId, getToken());
    await loadInfo();
...
formDelete.addEventListener("submit", async (e) => {
    e.preventDefault();
    const response = await deleteClassification(classificationId, getToken());
    await loadInfo();
    showAlertSuccess(response.success, response.data.general[0]);
});
...
btnEdit.addEventListener("click", async (e) => {
    e.preventDefault();
    window.location.href = `${baseUrlClassification}/${classificationId}/edit`;
    return;
});
...
const loadInfo = async () => {
    ...
    name.innerText = data.classification;
